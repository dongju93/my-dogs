$(function () {
    var IMP = window.IMP;
    IMP.init("imp11084289");
    $(".order-form").on("submit", function (e) {
        var amount = parseFloat(
            $('.order-form input[name="amount"]').val().replace(",", "")
        );
...
function AjaxCreateOrder(e) {
    ...
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
            alert("(11)페이지가 존재하지 않습니다.");
        } else if (jqXHR.status == 403) {
            alert("(11)로그인 해주세요.");
        } else {
            alert("(11)문제가 발생했습니다. 다시 시도해주세요.");
        }
    });
    return order_id;
}
...
function AjaxStoreTransaction(e, order_id, amount, type) {
    ...
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
            alert("(22)페이지가 존재하지 않습니다.");
        } else if (jqXHR.status == 403) {
            alert("(22)로그인 해주세요.");
        } else {
            alert("(22)문제가 발생했습니다. 다시 시도해주세요.");
        }
    });
    return merchant_id;
}
...
function ImpTransaction(e, order_id, merchant_id, imp_id, amount) {
    ...
    request.done(function (data) {
        if (data.works) {
            $(location).attr(
                "href",
                location.origin + order_complete_url + "?order_id=" + order_id
            );
        }
    });
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
            alert("(33)페이지가 존재하지 않습니다.");
        } else if (jqXHR.status == 403) {
            alert("(33)로그인 해주세요.");
        } else {
            alert("(33)문제가 발생했습니다. 다시 시도해주세요.");
        }
    });
}
